...
# -*- coding: utf-8 -*-
"""
STM Classification with Vision Mamba (Vim) - Variant 5: Compact Model
Phase 2: The Modern Sequence Model

This is a minimal/compact variant for ultra-fast training and baseline comparison:
...
- Same other params: d_conv=4, expand=2
- Increased batch size: batch_size=128 (vs 64, due to smaller memory footprint)
- Expected training time: ~12-15 min/epoch (vs 30-35 min for baseline - 2.5x faster)
- Total parameters: ~1.2M (vs ~8M in baseline - 85% reduction)

Goal: Establish minimal viable architecture and enable very fast experimentation
"""
...
class VisionMamba(nn.Module):
    """
    Vision Mamba for STM Classification - Compact/Minimal Variant
    
    Architecture:
    1. Patch Embedding (1x1 patches = each bin is a token)
    2. Positional Embedding (learnable, absolute positions)
    3. Stack of Vim Blocks (bidirectional SSM) - ONLY 6 LAYERS
    4. Global Average Pooling
    5. Classification Head
    
    Changes from baseline:
    - d_model: 96 (vs 192) - 50% REDUCTION for minimal model
    - depth: 6 (vs 12) - 50% REDUCTION for speed
    - d_state: 8 (vs 16) - 50% REDUCTION
    - drop_path_rate: 0.05 (vs 0.1) - REDUCED (shallower model needs less)
    - Total params: ~1.2M (vs ~8M - 85% smaller)
    """
    def __init__(self, seq_len=1220, num_classes=6, d_model=96, depth=6,
                 d_state=8, d_conv=4, expand=2, drop_path_rate=0.05, dropout=0.1):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_ViM5.py <mode>")
    ...
    # Create model
    print("\n" + "="*60)
    print("Creating Vision Mamba model (Compact/Minimal Variant)...")
    print("="*60)
    ...
    model = VisionMamba(
        ...
        drop_path_rate=0.05,# REDUCED from 0.1 (shallower model)
        dropout=0.1
    )
