...
# -*- coding: utf-8 -*-
"""
STM Classification with Vision Mamba (Vim) - Variant 4: Wider Model
Phase 2: The Modern Sequence Model

This is a wider variant optimized for richer per-layer representations:
- Increased model dimension: d_model=256 (vs 192 in baseline)
- Slightly reduced layers: depth=10 (vs 12 in baseline, for param balance)
- Larger SSM state: d_state=24 (vs 16 in baseline)
- Increased expand: expand=3 (vs 2 in baseline)
- Same batch size: 64 (may need to reduce to 48)
- Expected training time: ~35-40 min/epoch (vs 30-35 min for baseline)
- Total parameters: ~13M (vs ~8M in baseline)

Goal: Test whether wider representations per token improve feature quality
"""
...
class VisionMamba(nn.Module):
    """
    Vision Mamba for STM Classification - Wider Variant
    
    Architecture:
    1. Patch Embedding (1x1 patches = each bin is a token)
    2. Positional Embedding (learnable, absolute positions)
    3. Stack of Vim Blocks (bidirectional SSM) - 10 LAYERS (slightly reduced)
    4. Global Average Pooling
    5. Classification Head
    
    Changes from baseline:
    - d_model: 256 (vs 192) - INCREASED for richer representations
    - depth: 10 (vs 12) - REDUCED to balance parameter count
    - d_state: 24 (vs 16) - INCREASED for more SSM capacity
    - expand: 3 (vs 2) - INCREASED for wider FFN
    - drop_path_rate: 0.1 (same as baseline)
    - Total params: ~13M (vs ~8M)
    """
    def __init__(self, seq_len=1220, num_classes=6, d_model=256, depth=10,
                 d_state=24, d_conv=4, expand=3, drop_path_rate=0.1, dropout=0.1):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_ViM4.py <mode>")
    ...
    if mode == 0:
        ...
        directory = 'model/STM/ViM4_corpora_categories/standard'
    elif mode == 1:
        ds_nontonal_speech = True
        directory = 'model/STM/ViM4_corpora_categories/downsample'
    else:
    ...
    # Create model
    print("\n" + "="*60)
    print("Creating Vision Mamba model (Wider Variant)...")
    print("="*60)
    ...
    model = VisionMamba(
        ...
        depth=10,           # REDUCED from 12 (balance params)
        d_state=24,         # INCREASED from 16
        d_conv=4,           # Same as baseline
        expand=3,           # INCREASED from 2
        drop_path_rate=0.1, # Same as baseline
        dropout=0.1
    )
    ...
    print(f"Model size change: ~{total_params / 8000000:.1f}× vs baseline (wider but slightly shallower)")
