...
Compute:
    ...
    M_asym(i, s) = |P⁺(i, s)| - |P⁻_flipped(i, s)|        (Raw Direction)
...
def preprocess_stm_features(stm_2d):
    ...
    # Asymmetric: DIFFERENCE (NO RESCALING)
...
Dimension 2 (Channels):
    ...
    Channel 1: Asymmetric map (RAW, not rescaled)
        - Directional preference: |P+| - |P-|
        - Raw differences (can vary widely across samples)
        - Positive: down-sweeps dominate
        - Negative: up-sweeps dominate
        - Near zero: balanced or absent
        - Range: varies per sample, normalized via z-norm
```

**Key Changes from Previous Version**:
1. **Symmetric map**: AVERAGED instead of summed
   - Ensures similar magnitude to asymmetric map
   - More interpretable: represents mean energy across directions

2. **Asymmetric map**: NO RESCALING (changed from previous)
   - Raw directional differences preserved
   - Natural variation across samples maintained
   - Z-normalization (applied later) handles scale differences
...
- **NO rescaling on asymmetric map**: Preserves natural signal variation
- **Z-normalization handles scaling**: Applied per-sample after channel stacking
...
| **Asymmetric** | N/A | Raw difference: |P+| - |P-| |
| **Redundancy** | Full spectrum | Reduced |
| **Channel Balance** | N/A | Via z-normalization |
| **Params** | 1.52M | ~1.52M |
| **Conv Input Channels** | 1 | 2 |
| **Bio-Inspired** | No | Yes |
| **Expected F1** | 0.87+ | **0.87-0.90** |
...
```python
# After computing both maps with DC prepended
symmetric_with_dc = torch.cat([dc