...
from prepData import prepData_YAM as prepData # if this does not work, uncomment the sections below
...
if sys.argv[1]=='0':
    ...
    hm = hyperModel_drop()
    directory = "model/YAMNet/MLP_corpora_categories/Dropout/ROC-AUC/downsample"
    objective="val_auc"
elif sys.argv[1]=='5':
    train_dataset, val_dataset, test_dataset, n_feat, n_target = prepData(ds_nontonal_speech = True)
    hm = hyperModel_LN()
    directory = "model/YAMNet/MLP_corpora_categories/LayerNormalization/ROC-AUC/downsample"
    objective="val_auc"
elif sys.argv[1]=='6':
    train_dataset, val_dataset, test_dataset, n_feat, n_target = prepData(ds_nontonal_speech = True)
    hm = hyperModel_drop()
    directory = "model/YAMNet/MLP_corpora_categories/Dropout/macroF1/downsample"
    objective=kt.Objective("val_macro_f1_score", direction="max")
elif sys.argv[1]=='7':
    train_dataset, val_dataset, test_dataset, n_feat, n_target = prepData(ds_nontonal_speech = True)
    hm = hyperModel_LN()
    directory = "model/YAMNet/MLP_corpora_categories/LayerNormalization/macroF1/downsample"
    objective=kt.Objective("val_macro_f1_score", direction="max")
