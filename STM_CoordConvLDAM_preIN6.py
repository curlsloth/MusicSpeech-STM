...
Phase 2.6: Deeper Multi-Scale Fusion + Focal Loss + Earlier DRW

Key Innovations (V2.6 over V2.4):
1. Three-Scale Feature Fusion: Combines layer2 (128ch) + layer3 (256ch) + layer4 (512ch)
   - layer2 (128ch, 10×61): Fine-grained temporal patterns for music:non-vocal
   - layer3 (256ch, 5×31): Mid-level features
   - layer4 (512ch, 3×16): High-level semantics
   - Total fusion: 896ch → 512ch via 1×1 conv
2. Hybrid Loss: LDAM + Focal Loss (70% LDAM + 30% Focal)
   - Focal Loss (γ=2) focuses on hard-to-classify samples
   - Particularly helps music:non-vocal (high intra-class variance)
3. Earlier DRW Activation: Epoch 30 (instead of epoch 50)
   - More epochs with class reweighting benefits minority classes

Preserved from V2.4:
1. STM-Specific Augmentation: SpecAugment-style masking + axis shifts
2. Layer Freezing Strategy: Freeze layer1-2 for first 10 epochs
3. Attention Mechanisms: CA (layer1-2) + SE (layer3-4)
4. Discriminative Learning Rates: Pretrained layers learn slower
5. Difference Map Preprocessing: 2-channel input (Symmetric + Asymmetric)
6. ImageNet-pretrained ResNet-18 backbone

Target: 0.88-0.90 Macro F1 (surpass V2.4's 0.8682, approach SOTA 0.89+)
"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_CoordConvLDAM_preIN6.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/CoordConvLDAM_preIN6_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/CoordConvLDAM_preIN6_corpora_categories/downsample"
    else:
