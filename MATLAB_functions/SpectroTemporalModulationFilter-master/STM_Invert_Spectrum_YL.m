function [Recon, newTF,TFreference] = STM_Invert_Spectrum_YL(MSstruct, TF, varargin)
%
%   Invert the spectrum from a filtered TF (spectrogram) to a waveform
% [Recon, newTF,TFreference] = STM_Invert_Spectrum_YL(MSstruct,initialPhase,iter)
%
% adapted by Yike Li from the STM_invert_Spectrum.m by Adeen Flinker
% 
%       MSstruct      MS structure of the original sound file
%       TF            Time-frequency spectrogram matrix of the original sound file
%       Optional: 
%           initialPhase    initial phase 
%           iterOrnot   iterate or not
%           iter        times of iteration (if iterOrnot = 1)
%           debug       plot the generated TFs out (1) or not (0)

P = inputParser;
% TF = MSstruct.new_TF;       % Time frequency is the new (filtered) TF outputed by STM_Filter_Mod
P.addRequired('MSstruct');
P.addRequired('TF');
P.addOptional('initialPhase', []);
P.addOptional('iterORnot', 1);
P.addOptional('iter', 20);
P.addOptional('debug', 1);
parse(P, MSstruct, TF, varargin{:});

initialPhase = P.Results.initialPhase;
iterORnot = P.Results.iterORnot;
iter = P.Results.iter;
...
switch MSstruct.TF_Args.CB_Filter
    case 'gauss'
        ...
        newTF = TFreference.*exp(1i.*initialPhase);
    ...
    %                keyboard;
                ...
                newTF = TFreference.*exp(1i.*angle(newTF)); % use magnitude from the filtered TF and phase from the new TF


            end
        else
            thisSignal = zeros(1,size(TFreference,2));
            for ch=1:nchans
                clear thisTF thisBand;
%                 thisBand = inverseHilbert(TFreference(ch,:));
                thisBand = inverseHilbert(newTF(ch,:));
