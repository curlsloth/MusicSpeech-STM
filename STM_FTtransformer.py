...
- Per-feature learnable embeddings (2420 unique embeddings)
...
1. Treat STM as tabular data: 2420 continuous features
2. Each feature gets a unique learnable embedding
3. Attention discovers which bins co-occur for each class
4. Avoids spatial inductive biases of CNNs/Mamba
...
class prepData_STM_FTTransformer:
    ...
    Keeps STM features flattened as 2420-dimensional feature vectors.
    """
    def __init__(self, addAug=False, ds_nontonal_speech=False):
        ...
        # STM dimensions
        self.n_features = 2420  # Treat as 2420 separate features
...
class FeatureTokenizer(nn.Module):
    ...
    For STM:
        ...
        - ...
        - Feature 2419 (freq=19, time=120) → Embedding vector e_2419
...
class FTTransformer(nn.Module):
    ...
    Architecture:
    1. Feature Tokenizer: 2420 features → 2420 tokens (each with d_model dim)
    ...
    Example: Attention weight from Feature_250 (4Hz) to Feature_800 (2cyc/oct)
             reveals their correlation for "Speech" class
    """
    def __init__(self, n_features=2420, num_classes=6, d_model=192, n_heads=8,
...
if __name__ == "__main__":
    ...
    # Create data loaders
    # Reduced batch size to prevent OOM with O(L^2) attention on 2420 tokens
    batch_size = 32  # Reduced from 128 due to large attention matrix
