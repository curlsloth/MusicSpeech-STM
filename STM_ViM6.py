...
# -*- coding: utf-8 -*-
"""
STM Classification with Vision Mamba (Vim) - Variant 6: Regularized Compact Model
Phase 2: The Modern Sequence Model

This is a regularized version of ViM5 to combat overfitting observed in compact models:
- Minimal model dimension: d_model=96 (vs 192 in baseline - 50% reduction)
- Minimal layers: depth=6 (vs 12 in baseline - 50% reduction)
- Reduced SSM state: d_state=8 (vs 16 in baseline)
- ENHANCED REGULARIZATION:
  * Increased dropout: 0.2 (vs 0.1 in ViM5)
  * Increased drop_path_rate: 0.1 (vs 0.05 in ViM5)
  * Label smoothing: 0.1 (new, prevents overconfidence)
- Increased batch size: batch_size=128 (vs 64 baseline)
- Expected training time: ~12-15 min/epoch (same as ViM5)
- Total parameters: ~1.2M (same as ViM5)

Goal: Maintain ViM5 speed while improving generalization and preventing overfitting
Origin: Created after observing ViM5 val F1 peaked at epoch 20 then declined
"""
...
class VisionMamba(nn.Module):
    """
    Vision Mamba for STM Classification - Regularized Compact Variant
    
    Architecture:
    1. Patch Embedding (1x1 patches = each bin is a token)
    2. Positional Embedding (learnable, absolute positions)
    3. Stack of Vim Blocks (bidirectional SSM) - 6 LAYERS
    4. Global Average Pooling
    5. Classification Head with ENHANCED DROPOUT
    
    Changes from baseline:
    - d_model: 96 (vs 192) - 50% REDUCTION for speed
    - depth: 6 (vs 12) - 50% REDUCTION for speed
    - d_state: 8 (vs 16) - 50% REDUCTION
    - drop_path_rate: 0.1 (vs 0.1 baseline) - INCREASED from ViM5's 0.05
    - dropout: 0.2 (vs 0.1 baseline) - DOUBLED to prevent overfitting
    - Total params: ~1.2M (vs ~8M - 85% smaller)
    
    Regularization improvements over ViM5:
    - 2× dropout in classification head (0.1 → 0.2)
    - 2× drop_path for stochastic depth (0.05 → 0.1)
    - Label smoothing in loss (0.1)
    """
    def __init__(self, seq_len=1220, num_classes=6, d_model=96, depth=6,
                 d_state=8, d_conv=4, expand=2, drop_path_rate=0.1, dropout=0.2):
...
class BalancedSoftmaxLoss(nn.Module):
    """
    Balanced Softmax Loss with Label Smoothing for long-tailed recognition
    ...
    With label smoothing to prevent overconfidence (Szegedy et al., CVPR 2016):
        Soft targets = (1 - smoothing) * hard_label + smoothing / num_classes
    """
    def __init__(self, class_freq, label_smoothing=0.1):
        super(BalancedSoftmaxLoss, self).__init__()
        self.class_freq = torch.FloatTensor(class_freq)
        self.label_smoothing = label_smoothing
        
    def forward(self, logits, target):
        """Compute balanced softmax loss with label smoothing"""
        self.class_freq = self.class_freq.to(logits.device)
        adjusted_logits = logits + torch.log(self.class_freq + 1e-8)
        return nn.CrossEntropyLoss(label_smoothing=self.label_smoothing)(adjusted_logits, target)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_ViM6.py <mode>")
    ...
    # Create data loaders
    batch_size = 128  # INCREASED from 64 (compact model uses less memory)
    ...
    # Create model
    print("\n" + "="*60)
    print("Creating Vision Mamba model (Regularized Compact Variant)...")
    print("="*60)
    ...
    model = VisionMamba(
        ...
        d_model=96,         # REDUCED from 192 (50% smaller)
        depth=6,            # REDUCED from 12 (50% fewer layers)
        d_state=8,          # REDUCED from 16 (50% smaller)
        d_conv=4,           # Same as baseline
        expand=2,           # Same as baseline
        drop_path_rate=0.1, # INCREASED from ViM5's 0.05 to prevent overfitting
        dropout=0.2         # DOUBLED from 0.1 to combat overfitting
    )
    ...
    print(f"Regularization: dropout=0.2, drop_path=0.1, label_smoothing=0.1")
