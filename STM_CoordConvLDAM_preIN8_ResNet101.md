# STM_CoordConvLDAM_preIN8_ResNet101: Deeper Backbone for STM Classification

## Version Summary

**Version: 2.8-R101**  
**Base: V2.8 (preIN8)**  
**Key Innovation: ResNet-101 Backbone (replacing ResNet-18)**

## What's New in V2.8-R101

### 1. ResNet-101 Backbone (vs ResNet-18)

| Feature | ResNet-18 (V2.8) | ResNet-101 (V2.8-R101) |
|---------|------------------|------------------------|
| Layers | 18 | 101 |
| Block Type | BasicBlock (2 convs) | Bottleneck (3 convs) |
| Blocks per Stage | [2, 2, 2, 2] | [3, 4, 23, 3] |
| Parameters | ~11.7M | ~44.5M |
| Feature Dimension | 512 | 2048 |

**Channel Dimensions:**

| Layer | ResNet-18 | ResNet-101 |
|-------|-----------|------------|
| layer1 | 64 | 256 (64×4) |
...
#### Why ResNet-101?
- **More representational capacity**: 4× more parameters to model complex STM patterns
- **Deeper feature hierarchy**: 101 layers vs 18 layers for richer intermediate representations
- **Better ImageNet pretraining**: Benefits from more sophisticated transfer learning
- **Potential for higher accuracy**: Particularly for high-variance classes like `music:non-vocal`
...
Layer3 (23 Bottlenecks, 1024 ch, stride=2) + Squeeze-Excitation → feat_layer3
...
Each Bottleneck block in ResNet-101:
```
Input (C_in)
    ↓
...
| Parameter | V2.8 (ResNet-18) | V2.8-R101 (ResNet-101) |
|-----------|------------------|------------------------|
| Backbone | ResNet-18 | **ResNet-101** |
| Base LR | 1e-4 | 1e-4 |
...
python STM_CoordConvLDAM_preIN8_ResNet101.py 0

# Downsample mode (non-tonal speech downsampled to 100k)
python STM_CoordConvLDAM_preIN8_ResNet101.py 1
```

Checkpoints saved to:
- `model/STM/CoordConvLDAM_preIN8_ResNet101_corpora_categories/standard/ckpt/<timestamp>/`
- `model/STM/CoordConvLDAM_preIN8_ResNet101_corpora_categories/downsample/ckpt/<timestamp>/`
...
| Metric | V2.8 (ResNet-18) | V2.8-R101 Target |
|--------|------------------|------------------|
...
| Metric | V2.8 (ResNet-18) | V2.8-R101 |
|--------|------------------|-----------|
| Model Parameters | ~12M | ~45M |
| Batch Size | 256 | 128 |
| Est. GPU Memory | ~4-6 GB | ~12-16 GB |

**Note:** Reduced batch size is recommended to fit the larger model in GPU memory. If memory is still insufficient, consider:
- Further reducing batch size to 64
- Using gradient accumulation
- Using mixed precision training (torch.cuda.amp)

## Comparison: ResNet-18 vs ResNet-101

### Advantages of ResNet-101:
1. **Higher capacity**: Better modeling of complex, variable patterns
2. **Richer features**: 2048-dim vs 512-dim features
3. **More robust**: Deeper networks often generalize better
4. **Better pretrained weights**: ImageNet V2 weights for ResNet-101

### Potential Disadvantages:
1. **Slower training**: ~3-4× more FLOPs per forward pass
2. **More memory**: Larger batch sizes may not fit
3. **Risk of overfitting**: More parameters on limited data
4. **Longer convergence**: May need more epochs

### When to Use Each:
- **ResNet-18 (V2.8)**: Quick experiments, limited GPU memory, baseline
- **ResNet-101 (V2.8-R101)**: Final production model, when accuracy is critical
...
| **V2.8-R101** | **ResNet-101 Backbone** | **TBD** |
...
- **ResNet-152**: Marginal improvement over ResNet-101, significantly more memory
- **ResNet-50**: Good balance, but ResNet-101 offers better features
- **EfficientNet/ConvNeXt**: Could be explored in future versions
