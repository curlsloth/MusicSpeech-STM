...
# -*- coding: utf-8 -*-
"""
Enhanced Audio Spectrogram Mixer v5 (ASM-RH) for STM Classification

Builds on v4 with class weight removal:
1. Maintains symmetric STM processing from v4 (dimensionality reduction 121 → 61)
2. Removes class weighting to allow natural class distribution learning
3. Uses unweighted Focal Loss with contrastive regularization
4. Same model capacity as v4 (6 blocks, 160 dims)

Key Difference from v4:
- NO class weights (alpha=None in loss function)
- Relies on Focal Loss's inherent handling of class imbalance
- Simpler training dynamics without manual weight tuning
...
class EnhancedASM_RH_Classifier(nn.Module):
    """
    Enhanced ASM-RH v5 with symmetric STM processing (no class weights).
...
# ============================================================================
# Loss Function WITHOUT Class Weights (NEW in v5)
# ============================================================================
...
# ============================================================================
# Enhanced Trainer v5 (NO class weights)
# ============================================================================

class EnhancedTrainerV5:
    """Enhanced ASM trainer v5 - NO class weights"""
    ...
    def train(self, num_epochs, checkpoint_dir):
        ...
        print(f"\nStarting Enhanced ASM v5 training for {num_epochs} epochs...")
        print(f"Starting from epoch {self.start_epoch + 1}")
        print(f"Strategy: Symmetric STM + Unweighted Focal Loss + Contrastive")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STMasm_enhanced5.py <mode> [--resume <checkpoint_dir>]")
        sys.exit(1)
    ...
    if mode == 0:
        print("Mode 0: Standard training with symmetric STM processing (NO class weights)")
        ds_nontonal_speech = False
        directory = "model/STM/ASM_Enhanced5_corpora_categories/standard"
    elif mode == 1:
        print("Mode 1: Downsample non-tonal speech + symmetric STM (NO class weights)")
        ds_nontonal_speech = True
        directory = "model/STM/ASM_Enhanced5_corpora_categories/downsample"
    else:
    ...
    # Display class distribution (informational only, NO weights computed)
    ...
    print("NOTE: v5 does NOT use class weights - relying on Focal Loss for imbalance handling")
    ...
    # Create model with increased capacity (same as v4)
    print("\n" + "="*60)
    print("Creating Enhanced ASM-RH v5 model...")
    print("="*60)
    print(f"Architecture (same as v4):")
    print(f"  - Modulation rate dimension: 121 → {n_time_processed}")
    print(f"  - Model dimension: 160")
    print(f"  - Number of blocks: 6")
    print(f"Key difference from v4: NO class weights in loss function")
    ...
    # Create trainer v5 (NO class_weights parameter)
    trainer = EnhancedTrainerV5(
        model=model,
