...
# -*- coding: utf-8 -*-
"""
STM Classification with C3NeXt4: Focal-LDAM + Deeper Network + SE-Attention

Combines the best of C3NeXt2 and C3NeXt3:
1. Deeper network: [4, 12, 8] blocks (24 total) for better capacity
2. SE-Attention: Adaptive channel recalibration in each block
3. Focal-LDAM Loss: Hard example mining with class balancing
4. Earlier DRW: Starts at epoch 40
5. Stronger regularization: drop_path=0.2, head_dropout=0.4, mixup alpha=0.4

Target: Maximize performance on speech:tonal and music:non-vocal
Expected: Macro F1 0.860-0.870 (best of all variants)
"""
...
# ============================================================================
# C3NeXt4 Model: Deeper Network + SE-Attention
# ============================================================================

class C3NeXt4(nn.Module):
    """
    C3NeXt4: Combines deeper architecture with SE-attention
    
    Architecture:
    - Stem: CoordConv 4×4, stride 4
    - Stage 1: 4 ConvNeXt-SE blocks (channels=96)
    - Downsample 1: LayerNorm + Conv 2×2, stride 2
    - Stage 2: 12 ConvNeXt-SE blocks (channels=192)
    - Downsample 2: LayerNorm + Conv 2×2, stride 2
    - Stage 3: 8 ConvNeXt-SE blocks (channels=384)
    - Head: Global average pooling + LayerNorm + Linear
    
    Total: 24 ConvNeXt blocks + 24 SE modules
    """
    def __init__(self, num_classes=6, depths=[4, 12, 8], dims=[96, 192, 384],
                 drop_path_rate=0.2, layer_scale_init_value=1e-6, head_dropout=0.4, se_reduction=4):
        super().__init__()
...
class FocalLDAMLoss(nn.Module):
    """Combines Focal Loss with LDAM for hard example mining"""
    def __init__(self, cls_num_list, max_m=0.5, weight=None, s=30, gamma=2.0, label_smooth=0.05):
    ...
    def forward(self, x, target):
        ...
        if self.label_smooth > 0:
            ...
            loss = F.cross_entropy(output * self.s, target, reduction='none')
...
class Trainer:
    ...
    def train(self, num_epochs, checkpoint_dir):
        ...
        print(f"Architecture: C3NeXt4 (Deeper + SE-Attention)")
        print(f"Loss: Focal-LDAM (gamma=2.0)")
        print(f"DRW start: epoch 40")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_C3NeXt4.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/C3NeXt4_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/C3NeXt4_corpora_categories/downsample"
    else:
    ...
    print("\n" + "="*60)
    print("Creating C3NeXt4 (Deeper + SE-Attention)...")
    print("="*60)
    ...
    print(f"Architecture: C3NeXt4 - Deeper network + SE-Attention")
    print(f"Blocks: 24 ConvNeXt blocks [4, 12, 8] + 24 SE modules")
