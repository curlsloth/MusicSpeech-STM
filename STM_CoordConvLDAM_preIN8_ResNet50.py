...
# -*- coding: utf-8 -*-
"""
STM Classification with ImageNet-Pretrained ResNet-50 and LDAM Loss
Phase 2.8-R50: Center Loss with Medium Backbone

Key Innovations (V2.8-R50 over V2.8):
1. Medium-Depth Backbone: ResNet-50 (~25.6M params) instead of ResNet-18 (~11.7M params)
   - Balanced trade-off between capacity and efficiency
...
class BottleneckWithAttention(nn.Module):
    ...
    Used to add attention to pretrained ResNet-50 layers.
...
# ============================================================================
# Pretrained ResNet-50 with STM-Specific Adaptations + Attention + Multi-Scale
# ============================================================================

class PretrainedSTMResNet50(nn.Module):
    """
    ImageNet-pretrained ResNet-50 adapted for STM classification.
    
    V2.8-R50 Key Differences from V2.8 (ResNet-18):
    1. Medium-Depth Architecture: 50 layers with Bottleneck blocks (vs 18 layers with BasicBlocks)
    2. Larger Channel Dimensions: 256→512→1024→2048 (vs 64→128→256→512)
    3. More Parameters: ~25.6M (vs ~11.7M for ResNet-18)
    ...
    def __init__(self, num_classes=6, dropout=0.4, block_dropout=0.05, use_pretrained=True):
        super(PretrainedSTMResNet50, self).__init__()
        
        # Load pretrained ResNet-50 with error handling
        ...
        if use_pretrained:
            ...
            print("Creating ResNet-50 with random initialization...")
            pretrained_model = models.resnet50(weights=None)
            weights_loaded = False
        ...
        # Layer3: 6 Bottleneck blocks, add SE (channel selection, semantic features)
        ...
        # ===== Three-Scale Feature Fusion (V2.8-R50) =====
        ...
        self.fc = nn.Linear(2048, num_classes)  # 2048-dim features from ResNet-50
        ...
        if weights_loaded:
            print(f"✓ Created STM-adapted ResNet-50 V2.8-R50 with ImageNet pretrained weights ({num_classes} classes)")
        else:
            print(f"✓ Created STM-adapted ResNet-50 V2.8-R50 with random initialization ({num_classes} classes)")
        ...
        print(f"\nV2.8-R50 Configuration:")
        print(f"  • Backbone: ResNet-50 (Bottleneck blocks)")
    ...
    def forward(self, x, return_features=False):
        ...
        # ResNet blocks (note larger channel dimensions for ResNet-50)
...
class Trainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, 
        ...
        # Center Loss (V2.8-R50): Intra-class compactness
        # Feature dimension is 2048 (output before dropout/fc in PretrainedSTMResNet50)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_CoordConvLDAM_preIN8_ResNet50.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/CoordConvLDAM_preIN8_ResNet50_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/CoordConvLDAM_preIN8_ResNet50_corpora_categories/downsample"
    else:
    ...
    # Create model
    print("\n" + "="*60)
    print("Creating ImageNet-Pretrained ResNet-50 for STM (V2.8-R50)...")
    print("="*60)
    
    num_classes = 6
    model = PretrainedSTMResNet50(num_classes=num_classes, dropout=0.4, block_dropout=0.05)
    
    # Model info is printed in __init__
    print(f"\nV2.8-R50 Training Configuration:")
    print(f"  • Backbone: ResNet-50 (Bottleneck blocks)")
    print(f"  • Three-Scale Fusion: layer2 (512ch) + layer3 (1024ch) + layer4 (2048ch)")
    ...
    # Create trainer with V2.8-R50 hyperparameters
    trainer = Trainer(
        model=model,
    ...
    ax.set_title(f'STM Classification Confusion Matrix (V2.8-R50)\nNormalized by Row (Recall) | TTA F1: {tta_f1:.4f}', fontsize=14)
    ...
    # Print summary
    print("\n" + "="*60)
    print("SUMMARY (V2.8-R50 with ResNet-50 + Center Loss)")
    print("="*60)
