...
# -*- coding: utf-8 -*-
"""
STM Classification with ImageNet-Pretrained ResNet-101 and LDAM Loss
Phase 2.8-R101: Center Loss with Deeper Backbone

Key Innovations (V2.8-R101 over V2.8):
1. Deeper Backbone: ResNet-101 (44.5M params) instead of ResNet-18 (11.7M params)
   - More representational capacity for complex STM patterns
   - Bottleneck blocks (3 convs) instead of BasicBlocks (2 convs)
   - Channel dimensions: 256→512→1024→2048 (vs 64→128→256→512)
2. Larger Three-Scale Fusion: 512 + 1024 + 2048 = 3584 ch → 2048 ch
   - Richer feature representation for minority classes
3. Higher Feature Dimension: 2048-dim features (vs 512) for Center Loss

Preserved from V2.8:
1. Center Loss: Intra-class compactness (λ=0.1)
...
5. Layer Freezing Strategy: Freeze layer1-2 for first 10 epochs
6. Attention Mechanisms: CA (layer1-2) + SE (layer3-4)
7. Discriminative Learning Rates: Pretrained layers learn slower
8. Difference Map Preprocessing: 2-channel input (Symmetric + Asymmetric)
9. Integrated TTA Evaluation (5 augmentations)
10. Confusion Matrix Analysis

Note: Reduced batch size to 128 due to larger model memory requirements.

Target: 0.88+ Macro F1 (surpass V2.8 ResNet-18 baseline)
"""
...
class BottleneckWithAttention(nn.Module):
    """
    ResNet Bottleneck block with optional attention and dropout.
    Used to add attention to pretrained ResNet-101 layers.
    
    Note: Bottleneck blocks have conv1, conv2, conv3 (vs BasicBlock's conv1, conv2).
    The output channels are determined by conv3 (the final 1x1 conv).
    """
    expansion = 4
    
    def __init__(self, block, attention_type='CA', dropout=0.05):
        """
        Wraps an existing Bottleneck block and adds attention + dropout.
        
        Args:
            block: Existing nn.Module (Bottleneck from pretrained ResNet)
        ...
        # Bottleneck has conv3 as the last conv layer (1x1 conv restoring channels)
...
# ============================================================================
# Pretrained ResNet-101 with STM-Specific Adaptations + Attention + Multi-Scale
# ============================================================================

class PretrainedSTMResNet101(nn.Module):
    """
    ImageNet-pretrained ResNet-101 adapted for STM classification.
    
    V2.8-R101 Key Differences from V2.8 (ResNet-18):
    1. Deeper Architecture: 101 layers with Bottleneck blocks (vs 18 layers with BasicBlocks)
    2. Larger Channel Dimensions: 256→512→1024→2048 (vs 64→128→256→512)
    3. More Parameters: ~44.5M (vs ~11.7M for ResNet-18)
    4. Higher-Dimensional Features: 2048 (vs 512)
    
    Architecture Enhancements:
    1. Multi-Scale Feature Fusion: Combines layer2 (512ch) + layer3 (1024ch) + layer4 (2048ch)
       - Total: 3584ch fused to 2048ch
    ...
    Layer1 (CA) [256ch] → Layer2 (CA) [512ch] → Layer3 (SE) [1024ch] → Layer4 (SE) [2048ch] → 
    Multi-Scale Fusion [3584→2048] → Pool → FC
    """
    def __init__(self, num_classes=6, dropout=0.4, block_dropout=0.05, use_pretrained=True):
        super(PretrainedSTMResNet101, self).__init__()
        
        # Load pretrained ResNet-101 with error handling
        ...
        if use_pretrained:
            ...
            try:
                pretrained_model = models.resnet101(weights=ResNet101_Weights.IMAGENET1K_V2)
                weights_loaded = True
                print("✓ Successfully loaded ImageNet pretrained weights (V2)")
                ...
                try:
                    ...
                    pretrained_model = models.resnet101(weights=None)
                    weights_loaded = False
        else:
            print("Creating ResNet-101 with random initialization...")
            pretrained_model = models.resnet101(weights=None)
            weights_loaded = False
        ...
        # Layer1: 3 Bottleneck blocks, add CA (position-aware, early stage)
        # Output: 256 channels (64 * 4 expansion)
        self.layer1 = nn.Sequential(
            *[BottleneckWithAttention(block, attention_type='CA', dropout=block_dropout) 
              for block in pretrained_model.layer1]
        )
        
        # Layer2: 4 Bottleneck blocks, add CA (position still important)
        # Output: 512 channels (128 * 4 expansion)
        self.layer2 = nn.Sequential(
            *[BottleneckWithAttention(block, attention_type='CA', dropout=block_dropout) 
              for block in pretrained_model.layer2]
        )
        
        # Layer3: 23 Bottleneck blocks, add SE (channel selection, semantic features)
        # Output: 1024 channels (256 * 4 expansion)
        self.layer3 = nn.Sequential(
            *[BottleneckWithAttention(block, attention_type='SE', dropout=block_dropout) 
              for block in pretrained_model.layer3]
        )
        
        # Layer4: 3 Bottleneck blocks, add SE (channel selection, abstract features)
        # Output: 2048 channels (512 * 4 expansion)
        self.layer4 = nn.Sequential(
            *[BottleneckWithAttention(block, attention_type='SE', dropout=block_dropout) 
              for block in pretrained_model.layer4]
        )
        
        # ===== Three-Scale Feature Fusion (V2.8-R101) =====
        # Combines layer2 (512ch, 10×61) + layer3 (1024ch, 5×31) + layer4 (2048ch, 3×16)
        # Total: 512 + 1024 + 2048 = 3584 channels → fuse to 2048
        self.multi_scale_fusion = nn.Sequential(
            nn.Conv2d(512 + 1024 + 2048, 2048, kernel_size=1, bias=False),  # 3584 → 2048
            nn.BatchNorm2d(2048),
        ...
        self.fc = nn.Linear(2048, num_classes)  # 2048-dim features from ResNet-101
        ...
        if weights_loaded:
            print(f"✓ Created STM-adapted ResNet-101 V2.8-R101 with ImageNet pretrained weights ({num_classes} classes)")
        else:
            print(f"✓ Created STM-adapted ResNet-101 V2.8-R101 with random initialization ({num_classes} classes)")
        ...
        print(f"\nV2.8-R101 Configuration:")
        print(f"  • Backbone: ResNet-101 (Bottleneck blocks)")
        print(f"  • Three-Scale Fusion: layer2 (512ch) + layer3 (1024ch) + layer4 (2048ch)")
        print(f"  • Fusion output: 3584 → 2048 channels")
        print(f"  • Feature dimension: 2048 (for Center Loss)")
    ...
    def forward(self, x, return_features=False):
        ...
        # ResNet blocks (note larger channel dimensions for ResNet-101)
        x = self.layer1(x)  # (B, 256, 20, 121) - Bottleneck expansion 4x
        feat_layer2 = self.layer2(x)  # (B, 512, 10, 61) [stride=2 in layer2]
        feat_layer3 = self.layer3(feat_layer2)  # (B, 1024, 5, 31)  [stride=2 in layer3]
        feat_layer4 = self.layer4(feat_layer3)  # (B, 2048, 3, 16)  [stride=2 in layer4]
        
        # ===== Three-Scale Feature Fusion (V2.8-R101) =====
        # Upsample all features to match layer3 spatial dimensions (5, 31)
        ...
        feat_concat = torch.cat([feat_layer2_down, feat_layer3, feat_layer4_up], dim=1)  # (B, 3584, 5, 31)
        feat_fused = self.multi_scale_fusion(feat_concat)  # (B, 2048, 5, 31)
        
        # Global pooling
        x = self.avgpool(feat_fused)  # (B, 2048, 1, 1)
        x = torch.flatten(x, 1)  # (B, 2048)
        
        # Classification head
        feat = x  # Save features for analysis (2048-dim)
        x = self.dropout(x)
...
class Trainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, 
        ...
        # Center Loss (V2.8-R101): Intra-class compactness
        # Feature dimension is 2048 (output before dropout/fc in PretrainedSTMResNet101)
        self.center_loss = CenterLoss(num_classes=len(class_counts), feat_dim=2048).to(device)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_CoordConvLDAM_preIN8_ResNet101.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/CoordConvLDAM_preIN8_ResNet101_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/CoordConvLDAM_preIN8_ResNet101_corpora_categories/downsample"
    else:
    ...
    # Create data loaders
    # Reduced batch size (128 vs 256) due to larger model memory requirements
    batch_size = 128
    ...
    # Create model
    print("\n" + "="*60)
    print("Creating ImageNet-Pretrained ResNet-101 for STM (V2.8-R101)...")
    print("="*60)
    
    num_classes = 6
    model = PretrainedSTMResNet101(num_classes=num_classes, dropout=0.4, block_dropout=0.05)
    
    # Model info is printed in __init__
    print(f"\nV2.8-R101 Training Configuration:")
    print(f"  • Backbone: ResNet-101 (Bottleneck blocks)")
    print(f"  • Three-Scale Fusion: layer2 (512ch) + layer3 (1024ch) + layer4 (2048ch)")
    ...
    print(f"  • Center Loss: λ=0.1, feat_dim=2048")
    print(f"  • Early stopping: 20 epochs patience")
    
    # Create trainer with V2.8-R101 hyperparameters
    trainer = Trainer(
        model=model,
    ...
    ax.set_title(f'STM Classification Confusion Matrix (V2.8-R101)\nNormalized by Row (Recall) | TTA F1: {tta_f1:.4f}', fontsize=14)
    ...
    # Print summary
    print("\n" + "="*60)
    print("SUMMARY (V2.8-R101 with ResNet-101 + Center Loss)")
    print("="*60)
