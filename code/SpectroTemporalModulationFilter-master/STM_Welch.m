function [MSout] = STM_Welch(TFstruct,TemModFilter, SpecModFilter,Args,Type)
%
%  Caluclate modulation spectrum and filter
% 
%   MSout = STM_Filter_Welch(TFstruct, <TemModFilter, SpecModFilter,Args,Type>)
...
%       
%
%       adapted from: Adeen Flinker, Jan 2013 (adeen.f@gmail.com)
%       main change: use Welch's method instead of fft2     (Yike Li, Jul 2022)
...
if ~exist('Args') || isempty(Args)
    ...
    Args.fs = 16000; % sampling rate = 16000 hz
    Args.windowlength = 4; % window length = 4s
    Args.overlap = 0.5; % window overlap = 50%
end
...
% orig_ms = fft2(TF);
orig_ms = my_Welch(TF, Args.fs, Args.windowlength, Args.overlap); % use my_Welch instead of fft2 to deal with unequal length input

% [Nfr,Ntm] = size(TF);
[Nfr, Ntm] = size(orig_ms);
fstep = Args.MS_fstep;
