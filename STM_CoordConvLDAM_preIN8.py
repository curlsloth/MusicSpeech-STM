...
Phase 2.8: Center Loss for Intra-Class Compactness

Key Innovations (V2.8 over V2.6):
1. Center Loss: Penalizes distance between features and learned class centers
   - L_center = Σ ||features_i - center[y_i]||²
   - L_total = L_hybrid + λ × L_center (λ=0.1)
   - Encourages intra-class compactness in feature space
   - Particularly beneficial for high-variance classes like music:non-vocal
2. Integrated TTA Evaluation: Test-Time Augmentation (5 augmentations)
   - Original, time_flip, freq_shift±2, time_shift+5
   - Soft voting on logits for robust predictions
3. Confusion Matrix Analysis: Detailed per-class confusion analysis

Preserved from V2.6:
1. Three-Scale Feature Fusion: layer2 (128ch) + layer3 (256ch) + layer4 (512ch)
...
9. ImageNet-pretrained ResNet-18 backbone
...
class PretrainedSTMResNet18(nn.Module):
    ...
    def __init__(self, num_classes=6, dropout=0.4, block_dropout=0.05, use_pretrained=True):
        ...
        # Combines layer2 (128ch, 10×61) + layer3 (256ch, 5×31) + layer4 (512ch, 3×16)
        # layer2 provides fine-grained temporal patterns crucial for music:non-vocal
        self.multi_scale_fusion = nn.Sequential(
            nn.Conv2d(128 + 256 + 512, 512, kernel_size=1, bias=False),  # 896 → 512
            nn.BatchNorm2d(512),
        ...
        if weights_loaded:
            print(f"✓ Created STM-adapted ResNet-18 V2.6 with ImageNet pretrained weights ({num_classes} classes)")
        else:
            print(f"✓ Created STM-adapted ResNet-18 V2.6 with random initialization ({num_classes} classes)")
        ...
        print(f"\nV2.6 Enhancements:")
        print(f"  • Three-Scale Fusion: layer2 (128ch) + layer3 (256ch) + layer4 (512ch)")
        print(f"  • Hybrid Loss: LDAM + Focal Loss (70%/30%)")
        print(f"  • Earlier DRW: Epoch 30 (vs 50 in V2.4)")
    ...
    def forward(self, x, return_features=False):
        ...
        feat_layer2 = self.layer2(x)  # (B, 128, 10, 61) [stride=2 in layer2]
        feat_layer3 = self.layer3(feat_layer2)  # (B, 256, 5, 31)  [stride=2 in layer3]
        ...
        feat_concat = torch.cat([feat_layer2_down, feat_layer3, feat_layer4_up], dim=1)  # (B, 896, 5, 31)
...
class Trainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, 
        ...
        # Feature dimension is 512 (output before dropout/fc in PretrainedSTMResNet18)
        self.center_loss = CenterLoss(num_classes=len(class_counts), feat_dim=512).to(device)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_CoordConvLDAM_preIN8.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/CoordConvLDAM_preIN8_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/CoordConvLDAM_preIN8_corpora_categories/downsample"
    else:
    ...
    ax.set_title(f'STM Classification Confusion Matrix (V2.8)\nNormalized by Row (Recall) | TTA F1: {tta_f1:.4f}', fontsize=14)
    ...
    # Print summary
    print("\n" + "="*60)
    print("SUMMARY (V2.8 with Center Loss)")
    print("="*60)
