...
Phase 5: Enhanced Training with Class-Balanced Sampling & SpecAugment

Key Innovations (V5 = V3 + Targeted Improvements):
1. Earlier DRW Activation: Epoch 30 (vs V3's epoch 50) - more time to rebalance
2. Class-Balanced Sampling: Equal class sampling per epoch via WeightedRandomSampler
3. SpecAugment Masking: Time/frequency masking for better generalization
4. All V3 features: Attention, discriminative LR, moderate regularization

Motivation (from V3 results - Test F1: 0.8646):
- music:non-vocal: 0.67 F1 (worst) → limited exposure during training
- speech:tonal: 0.72 recall → DRW activated too late (epoch 51)
- Minority classes need earlier reweighting and balanced exposure
...
5. V2 LDAM Loss implementation

Target: 0.88-0.90 Macro F1 (beat V3's 0.8646)
- music:non-vocal: 0.70-0.75 F1 (current: 0.67)
- speech:tonal: 0.75-0.80 recall (current: 0.72)
"""
...
class Trainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, 
        ...
        # V5: Earlier DRW activation
        self.drw_start_epoch = 30  # V3 used 50, V5 uses 30
    ...
    def train_epoch(self, epoch, total_epochs, use_drw=False, use_mixup=True, use_specaugment=True):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            # V5: Apply SpecAugment with 50% probability
            if use_specaugment and np.random.rand() < 0.5:
                data = spec_augment(data, time_mask_param=10, freq_mask_param=3, 
    ...
    def train(self, num_epochs, checkpoint_dir):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            # V5: Earlier DRW activation (epoch 30 instead of 50)
            use_drw = epoch > self.drw_start_epoch
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_CoordConvLDAM_preIN5.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/CoordConvLDAM_preIN5_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/CoordConvLDAM_preIN5_corpora_categories/downsample"
    else:
    ...
    print(f"  \u2022 Earlier DRW: Activated at epoch 31 (vs V3's epoch 51)")
    print(f"  \u2022 Class-Balanced Sampling: WeightedRandomSampler")
    print(f"  \u2022 SpecAugment: Time mask (10 bins), Freq mask (3 bins)")
    ...
    # Create trainer with V5 enhancements
    trainer = Trainer(
        model=model,
