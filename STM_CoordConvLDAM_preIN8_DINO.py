...
# -*- coding: utf-8 -*-
"""
STM Classification with Self-Supervised DINO (ViT-Small) and LDAM Loss
Phase 2.8-DINO: Center Loss with Vision Transformer Backbone

Key Innovations (V2.8-DINO):
1. DINO ViT-Small Backbone: Self-supervised Vision Transformer (ICCV 2021)
   - Self-supervised pretraining without labels
   - Better semantic features than supervised pretraining
   - Patch-based input processing (flexible for small inputs)
   - 384-dim features from [CLS] token or patch averaging
   - ~22M parameters
...
DINO-Specific Adaptations:
1. Custom Patch Embedding: Adapted for 2-channel (20×121) STM input
2. Positional Encoding: Interpolated for non-square input
3. Multi-Scale Features: From intermediate transformer blocks
4. Hybrid Loss: LDAM + Focal Loss (70% LDAM + 30% Focal)
5. Earlier DRW Activation: Epoch 30
6. STM-Specific Augmentation: SpecAugment-style masking + axis shifts
7. Layer Freezing Strategy: Freeze first 6 transformer blocks for 10 epochs
8. Discriminative Learning Rates: Pretrained layers learn slower
9. Difference Map Preprocessing: 2-channel input (Symmetric + Asymmetric)
...
# DINO is loaded via torch.hub from facebookresearch/dino
...
# ============================================================================
# Pretrained DINO ViT-Small with STM-Specific Adaptations
# ============================================================================

class PretrainedSTMDINO(nn.Module):
    """
    Self-supervised DINO ViT-Small adapted for STM classification.
    
    DINO (Self-DIstillation with NO labels) is a self-supervised Vision Transformer
    from Facebook AI Research (ICCV 2021) that learns semantic features without labels.
    
    V2.8-DINO Architecture:
    1. Custom Patch Embedding: Projects 2-channel STM (20×121) to patch tokens
       - Uses small patches (4×4) to preserve STM spatial details
       - Handles non-square input natively
    2. DINO ViT-Small Transformer: 12 blocks, 384-dim, 6 heads
    3. Multi-Scale Features: Extracts features from blocks 4, 8, 12
       - Early blocks: low-level patterns
       - Middle blocks: mid-level structures  
       - Late blocks: high-level semantics
    4. Feature Fusion: Concatenate multi-scale features → 384*3 = 1152 → 384
    5. Classification Head: 384-dim → num_classes
    
    Key Modifications:
    1. Patch Embed: 2-channel input with coordinate channels (4 total)
    2. Position Encoding: Interpolated for custom input size
    3. [CLS] Token: Used for classification with multi-scale aggregation
    4. DropPath: Built-in stochastic depth in transformer blocks
    
    Architecture Flow:
    Input (B, 2, 20, 121) → Patch Embed (B, N_patches, 384) → 
    [CLS] + Pos Embed → 12 Transformer Blocks (extract at 4, 8, 12) → 
    Multi-Scale Fusion → Classification Head
    """
    def __init__(self, num_classes=6, dropout=0.4, use_pretrained=True):
        super(PretrainedSTMDINO, self).__init__()
        
        # Configuration
        self.embed_dim = 384  # ViT-Small hidden dimension
        self.num_heads = 6
        self.num_blocks = 12
        self.patch_size = 4  # Smaller patches for small input
        self.in_channels = 4  # 2 STM + 2 coordinate channels
        ...
        self.num_patches_h = self.input_h // self.patch_size  # 20 // 4 = 5
        self.num_patches_w = self.input_w // self.patch_size  # 121 // 4 = 30 (pad to 124 for divisibility)
        self.padded_w = (self.input_w + self.patch_size - 1) // self.patch_size * self.patch_size  # 124
        self.num_patches = self.num_patches_h * (self.padded_w // self.patch_size)  # 5 * 31 = 155
        
        # Load pretrained DINO ViT-Small
        weights_loaded = False
        
        if use_pretrained:
            print("Loading self-supervised DINO ViT-Small...")
            try:
                # Load DINO from torch.hub
                dino_model = torch.hub.load('facebookresearch/dino:main', 'dino_vits16', pretrained=True)
                weights_loaded = True
                print("✓ Successfully loaded DINO ViT-Small pretrained weights")
            except Exception as e:
                print(f"⚠ Warning: Failed to download DINO weights: {e}")
                print("⚠ Creating model from scratch...")
                dino_model = None
                weights_loaded = False
        else:
            print("Creating DINO ViT-Small with random initialization...")
            dino_model = None
            weights_loaded = False
        
        # ===== Custom Patch Embedding for STM =====
        # CoordConv-style: Add coordinate channels to input
        self.coord_conv = nn.Sequential(
            nn.Conv2d(2, 4, kernel_size=1, bias=False),  # Expand to 4 channels
        )
        
        # Patch embedding: (B, 4, 20, 124) → (B, 155, 384)
        ...
        # Transformer blocks (from DINO or custom)
        if weights_loaded and dino_model is not None:
            # Copy transformer blocks from DINO
            self.blocks = dino_model.blocks
            self.norm = dino_model.norm
            
            # Copy class token initialization
            with torch.no_grad():
                self.cls_token.data = dino_model.cls_token.data.clone()
                
                # Interpolate position embedding for our patch count
                orig_pos_embed = dino_model.pos_embed.data  # (1, 197, 384) for 14×14 patches + cls
                # Extract cls pos embed
                cls_pos_embed = orig_pos_embed[:, 0:1, :]  # (1, 1, 384)
                patch_pos_embed = orig_pos_embed[:, 1:, :]  # (1, 196, 384)
                
                # Reshape to 2D and interpolate
                orig_size = int(patch_pos_embed.shape[1] ** 0.5)  # 14
                patch_pos_embed = patch_pos_embed.reshape(1, orig_size, orig_size, self.embed_dim)
                patch_pos_embed = patch_pos_embed.permute(0, 3, 1, 2)  # (1, 384, 14, 14)
                
                # Interpolate to our patch grid
                patch_pos_embed = F.interpolate(
                    patch_pos_embed,
                    size=(self.num_patches_h, self.padded_w // self.patch_size),
                    mode='bilinear',
                    align_corners=False
                )
                patch_pos_embed = patch_pos_embed.permute(0, 2, 3, 1)  # (1, 5, 31, 384)
                patch_pos_embed = patch_pos_embed.reshape(1, -1, self.embed_dim)  # (1, 155, 384)
                
                # Concatenate cls and patch pos embeds
                self.pos_embed.data = torch.cat([cls_pos_embed, patch_pos_embed], dim=1)
                
            print("✓ Interpolated position embeddings for STM input size")
        else:
        ...
        # Extract from blocks 4, 8, 12 (0-indexed: 3, 7, 11)
        self.fusion_blocks = [3, 7, 11]  # Block indices for multi-scale features
        
        # Fusion: 384 * 3 = 1152 → 384
        ...
        if weights_loaded:
            print(f"✓ Created STM-adapted DINO ViT-Small with pretrained weights ({num_classes} classes)")
        else:
            print(f"✓ Created STM-adapted DINO ViT-Small with random initialization ({num_classes} classes)")
            print(f"  Note: Training from scratch may require more epochs")
        ...
        print(f"\nV2.8-DINO Architecture:")
        print(f"  • Backbone: DINO ViT-Small (384-dim, 6 heads, 12 blocks)")
        print(f"  • Patch size: {self.patch_size}×{self.patch_size}")
        print(f"  • Num patches: {self.num_patches} ({self.num_patches_h}×{self.padded_w // self.patch_size})")
        print(f"  • Multi-Scale Fusion: blocks 4, 8, 12 → 1152 → 384")
        print(f"  • Feature dimension: {self.embed_dim}")
        print(f"  • Head dropout: {dropout}")
        print(f"  • Layer freezing support (first 10 epochs)")
    
    def add_coord_channels(self, x):
        """Add coordinate channels to input (like CoordConv)."""
        b, c, h, w = x.shape
    ...
    def freeze_early_layers(self):
        """Freeze patch embed and first 6 transformer blocks for early training."""
        for param in self.patch_embed.parameters():
            param.requires_grad = False
        for param in self.coord_conv.parameters():
            param.requires_grad = False
        
        # Freeze first 6 blocks (half of transformer)
        for i, block in enumerate(self.blocks):
            if i < 6:
                for param in block.parameters():
                    param.requires_grad = False
        
        print("✓ Frozen: coord_conv, patch_embed, transformer blocks 0-5")
    ...
    def forward(self, x, return_features=False):
        ...
        # Add coordinate channels: (B, 2, 20, 121) → (B, 4, 20, 121)
        x = self.add_coord_channels(x)
        
        # Pad width to be divisible by patch_size: 121 → 124
        if x.shape[3] % self.patch_size != 0:
            pad_w = self.patch_size - (x.shape[3] % self.patch_size)
            # For 4D tensors, reflect padding needs (left, right, top, bottom) format
            x = F.pad(x, (0, pad_w, 0, 0), mode='reflect')  # (B, 4, 20, 124)
        
        # Patch embedding: (B, 4, 20, 124) → (B, 384, 5, 31) → (B, 155, 384)
        x = self.patch_embed(x)  # (B, 384, 5, 31)
        x = x.flatten(2).transpose(1, 2)  # (B, 155, 384)
        
        # Add class token: (B, 156, 384)
        ...
        # Forward through transformer blocks, collecting multi-scale features
        multi_scale_features = []
        
        for i, block in enumerate(self.blocks):
            x = block(x)
            if i in self.fusion_blocks:
                # Extract [CLS] token feature for this scale
                multi_scale_features.append(x[:, 0])  # (B, 384)
        
        # Final normalization
        x = self.norm(x)
        
        # Multi-scale fusion: concat features from blocks 4, 8, 12
        feat_concat = torch.cat(multi_scale_features, dim=1)  # (B, 1152)
        feat_fused = self.multi_scale_fusion(feat_concat)  # (B, 384)
        
        # Classification head
        feat = feat_fused  # Save features for Center Loss
        x = self.dropout(feat)
...
class Trainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, 
        ...
        # Discriminative Learning Rates for DINO ViT-Small
        # Collect parameters carefully to avoid overlap
        
        # DINO structure:
        # - coord_conv + patch_embed: input processing
        # - cls_token + pos_embed: position encoding
        # - blocks[0-5]: early transformer blocks (frozen first 10 epochs)
        # - blocks[6-11]: late transformer blocks
        # - norm: final layer norm
        # - multi_scale_fusion: feature fusion
        # - fc: classification head
        
        embed_params = list(model.coord_conv.parameters()) + list(model.patch_embed.parameters())
        ...
        for i, block in enumerate(model.blocks):
            if i < 6:
                early_block_params.extend(list(block.parameters()))
            else:
                late_block_params.extend(list(block.parameters()))
        ...
        param_groups = [
            # Embedding + Early Blocks: 0.1x (heavily pretrained, minimal adaptation)
            {'params': embed_params + early_block_params,
             'lr': lr * 0.1, 'name': 'embed_early_blocks'},
            
            # Late Blocks: 0.5x (pretrained, moderate adaptation)
            {'params': late_block_params,
             'lr': lr * 0.5, 'name': 'late_blocks'},
            
            # Norm: 1.0x (adapt to new data distribution)
            {'params': norm_params,
             'lr': lr * 1.0, 'name': 'norm'},
        ...
        # Feature dimension is 384 (output before dropout/fc in PretrainedSTMDINO)
        self.center_loss = CenterLoss(num_classes=len(class_counts), feat_dim=384).to(device)
        ...
        if freeze_epochs > 0:
            ...
            print(f"\n→ Patch embed + blocks 0-5 frozen for first {freeze_epochs} epochs")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python STM_CoordConvLDAM_preIN8_DINO.py <mode>")
    ...
    if mode == 0:
        ...
        directory = "model/STM/CoordConvLDAM_preIN8_DINO_corpora_categories/standard"
    elif mode == 1:
        ds_nontonal_speech = True
        directory = "model/STM/CoordConvLDAM_preIN8_DINO_corpora_categories/downsample"
    else:
    ...
    # Create data loaders
    batch_size = 64  # Reduced from 256 for larger DINO ViT-Small model
    ...
    # Create model
    print("\n" + "="*60)
    print("Creating Self-Supervised DINO ViT-Small for STM (V2.8-DINO)...")
    print("="*60)
    
    num_classes = 6
    model = PretrainedSTMDINO(num_classes=num_classes, dropout=0.4)
    
    # Model info is printed in __init__
    print(f"\nV2.8-DINO Training Configuration:")
    print(f"  \u2022 Multi-Scale Fusion: blocks 4, 8, 12 → 1152 → 384")
    print(f"  \u2022 Discriminative LR: Embed/Early (0.1x), Late (0.5x), Head (1.0x)")
    print(f"  \u2022 Layer Freezing: patch_embed + blocks 0-5 frozen for first 10 epochs")
    ...
    # Create trainer with V2.8-DINO hyperparameters
    trainer = Trainer(
        model=model,
        ...
        lr=1e-4,
        weight_decay=5e-4,
        freeze_epochs=10  # Freeze patch_embed + blocks 0-5 for first 10 epochs
    )
    ...
    ax.set_title(f'STM Classification Confusion Matrix (V2.8-DINO)\nNormalized by Row (Recall) | TTA F1: {tta_f1:.4f}', fontsize=14)
    ...
    # Print summary
    print("\n" + "="*60)
    print("SUMMARY (V2.8-DINO with ViT-Small + Center Loss)")
    print("="*60)
